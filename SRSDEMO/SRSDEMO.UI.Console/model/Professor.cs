...
public class Professor : Person
{
    ...
    public void AgreeToTeach(Section s)
    {
        ...
        for (int n = 0; n < Teaches.Count; n++)
        {
            if (Teaches[n].OfferedIn == s.OfferedIn)
            {
                if (Teaches[n].DayOfWeek == s.DayOfWeek)
                {
                    if (Teaches[n].TimeOfDay == s.TimeOfDay)
                    {
                        isSameTime = false;
                    }
                    else continue;
        ...
        {
            Console.WriteLine("同一位教授不能在同一时刻讲两门课！");
        }
